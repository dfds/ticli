/*
SelfService API

SelfService API

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the MembershipApprovalApiResource type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &MembershipApprovalApiResource{}

// MembershipApprovalApiResource struct for MembershipApprovalApiResource
type MembershipApprovalApiResource struct {
	Id NullableString `json:"id,omitempty"`
	ApprovedBy NullableString `json:"approvedBy,omitempty"`
	ApprovedAt NullableString `json:"approvedAt,omitempty"`
}

// NewMembershipApprovalApiResource instantiates a new MembershipApprovalApiResource object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMembershipApprovalApiResource() *MembershipApprovalApiResource {
	this := MembershipApprovalApiResource{}
	return &this
}

// NewMembershipApprovalApiResourceWithDefaults instantiates a new MembershipApprovalApiResource object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMembershipApprovalApiResourceWithDefaults() *MembershipApprovalApiResource {
	this := MembershipApprovalApiResource{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MembershipApprovalApiResource) GetId() string {
	if o == nil || IsNil(o.Id.Get()) {
		var ret string
		return ret
	}
	return *o.Id.Get()
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MembershipApprovalApiResource) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Id.Get(), o.Id.IsSet()
}

// HasId returns a boolean if a field has been set.
func (o *MembershipApprovalApiResource) HasId() bool {
	if o != nil && o.Id.IsSet() {
		return true
	}

	return false
}

// SetId gets a reference to the given NullableString and assigns it to the Id field.
func (o *MembershipApprovalApiResource) SetId(v string) {
	o.Id.Set(&v)
}
// SetIdNil sets the value for Id to be an explicit nil
func (o *MembershipApprovalApiResource) SetIdNil() {
	o.Id.Set(nil)
}

// UnsetId ensures that no value is present for Id, not even an explicit nil
func (o *MembershipApprovalApiResource) UnsetId() {
	o.Id.Unset()
}

// GetApprovedBy returns the ApprovedBy field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MembershipApprovalApiResource) GetApprovedBy() string {
	if o == nil || IsNil(o.ApprovedBy.Get()) {
		var ret string
		return ret
	}
	return *o.ApprovedBy.Get()
}

// GetApprovedByOk returns a tuple with the ApprovedBy field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MembershipApprovalApiResource) GetApprovedByOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ApprovedBy.Get(), o.ApprovedBy.IsSet()
}

// HasApprovedBy returns a boolean if a field has been set.
func (o *MembershipApprovalApiResource) HasApprovedBy() bool {
	if o != nil && o.ApprovedBy.IsSet() {
		return true
	}

	return false
}

// SetApprovedBy gets a reference to the given NullableString and assigns it to the ApprovedBy field.
func (o *MembershipApprovalApiResource) SetApprovedBy(v string) {
	o.ApprovedBy.Set(&v)
}
// SetApprovedByNil sets the value for ApprovedBy to be an explicit nil
func (o *MembershipApprovalApiResource) SetApprovedByNil() {
	o.ApprovedBy.Set(nil)
}

// UnsetApprovedBy ensures that no value is present for ApprovedBy, not even an explicit nil
func (o *MembershipApprovalApiResource) UnsetApprovedBy() {
	o.ApprovedBy.Unset()
}

// GetApprovedAt returns the ApprovedAt field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MembershipApprovalApiResource) GetApprovedAt() string {
	if o == nil || IsNil(o.ApprovedAt.Get()) {
		var ret string
		return ret
	}
	return *o.ApprovedAt.Get()
}

// GetApprovedAtOk returns a tuple with the ApprovedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MembershipApprovalApiResource) GetApprovedAtOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ApprovedAt.Get(), o.ApprovedAt.IsSet()
}

// HasApprovedAt returns a boolean if a field has been set.
func (o *MembershipApprovalApiResource) HasApprovedAt() bool {
	if o != nil && o.ApprovedAt.IsSet() {
		return true
	}

	return false
}

// SetApprovedAt gets a reference to the given NullableString and assigns it to the ApprovedAt field.
func (o *MembershipApprovalApiResource) SetApprovedAt(v string) {
	o.ApprovedAt.Set(&v)
}
// SetApprovedAtNil sets the value for ApprovedAt to be an explicit nil
func (o *MembershipApprovalApiResource) SetApprovedAtNil() {
	o.ApprovedAt.Set(nil)
}

// UnsetApprovedAt ensures that no value is present for ApprovedAt, not even an explicit nil
func (o *MembershipApprovalApiResource) UnsetApprovedAt() {
	o.ApprovedAt.Unset()
}

func (o MembershipApprovalApiResource) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o MembershipApprovalApiResource) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Id.IsSet() {
		toSerialize["id"] = o.Id.Get()
	}
	if o.ApprovedBy.IsSet() {
		toSerialize["approvedBy"] = o.ApprovedBy.Get()
	}
	if o.ApprovedAt.IsSet() {
		toSerialize["approvedAt"] = o.ApprovedAt.Get()
	}
	return toSerialize, nil
}

type NullableMembershipApprovalApiResource struct {
	value *MembershipApprovalApiResource
	isSet bool
}

func (v NullableMembershipApprovalApiResource) Get() *MembershipApprovalApiResource {
	return v.value
}

func (v *NullableMembershipApprovalApiResource) Set(val *MembershipApprovalApiResource) {
	v.value = val
	v.isSet = true
}

func (v NullableMembershipApprovalApiResource) IsSet() bool {
	return v.isSet
}

func (v *NullableMembershipApprovalApiResource) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMembershipApprovalApiResource(val *MembershipApprovalApiResource) *NullableMembershipApprovalApiResource {
	return &NullableMembershipApprovalApiResource{value: val, isSet: true}
}

func (v NullableMembershipApprovalApiResource) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMembershipApprovalApiResource) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


